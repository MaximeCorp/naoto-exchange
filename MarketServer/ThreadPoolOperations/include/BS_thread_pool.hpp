...
// generate comparison operators. In C++17 we have to define them manually.
    ...
    static_assert(thread_pool_version == version(BS_THREAD_POOL_MODULE),
                  "The versions of BS.thread_pool.cppm and BS_thread_pool.hpp "
                  "do not match. Aborting compilation.");
    /**
     * @brief A flag indicating whether the thread pool library was compiled as
     * a C++20 module.
     */
    ...
    /**
     * @brief A flag indicating whether the thread pool library imported the
     * C++23 Standard Library module using `import std`.
     */
    ...
    /**
     * @brief The type used for the bitmask template parameter of the thread
     * pool.
     */
...
#    define BS_THREAD_POOL_INIT_FUNC_CONCEPT(F)                                \
        typename F,                                                            \
            typename = std::enable_if_t                                        \
                < std::is_invocable_v<F> || std::is_invocable_v < F,           \
            std::size_t >> // NOLINT(bugprone-macro-parentheses)
#endif

    /**
     * @brief A helper class to facilitate waiting for and/or getting the
     * results of multiple futures at once.
     *
    ...
    public:
         ...
         */
        [[nodiscard]] std::conditional_t<std::is_void_v<T>, void,
                                         std::vector<T>>
        get()
        {
        ...
        /**
         * @brief Check how many of the futures stored in this
         * `BS::multi_future` are ready.
         *
         ...
         * valid.
         *
         * @return `true` if all futures are valid, `false` if at least one of
         * the futures is not valid.
         */
        ...
        /**
         * @brief Wait for all the futures stored in this `BS::multi_future`,
         * but stop waiting after the specified duration has passed. This
         * function first waits for the first future for the desired duration.
         * If that future is ready before the duration expires, this function
         * waits for the second future for whatever remains of the duration. It
         * continues similarly until the duration expires.
         *
         * @tparam R An arithmetic type representing the number of ticks to
         * wait.
         * @tparam P An `std::ratio` representing the length of each tick in
         * seconds.
         * @param duration The amount of time to wait.
         * @return `true` if all futures have been waited for before the
         * duration expired, `false` otherwise.
         */
        ...
        /**
         * @brief Wait for all the futures stored in this `BS::multi_future`,
         * but stop waiting after the specified time point has been reached.
         * This function first waits for the first future until the desired time
         * point. If that future is ready before the time point is reached, this
         * function waits for the second future until the desired time point. It
         * continues similarly until the time point is reached.
         *
         ...
         * @return `true` if all futures have been waited for before the time
         * point was reached, `false` otherwise.
         */
     ...
     * `detach_blocks()`, `submit_blocks()`, `detach_loop()`, and
     * `submit_loop()`.
     *
     * @tparam T The type of the indices. Should be a signed or unsigned
     * integer.
     */
    template <typename T>
    ...
    public:
         ...
         * @param num_blocks_ The desired number of blocks to divide the range
         * into.
         */
        ...
        /**
         * @brief Get the number of blocks. Note that this may be different than
         * the desired number of blocks that was passed to the constructor.
         *
    ...
    private:
        ...
        /**
         * @brief The remainder obtained after dividing the total size by the
         * number of blocks.
         */
    ...
    {
        wait_deadlock()
            : std::runtime_error("BS::wait_deadlock"){};
    };
#endif
...
#    endif
    ...
    /**
     * @brief Get the processor affinity of the current process using the
     * current platform's native API. This should work on Windows and Linux, but
     * is not possible on macOS as the native API does not allow it.
     *
...
#    endif
    ...
    /**
     * @brief Set the processor affinity of the current process using the
     * current platform's native API. This should work on Windows and Linux, but
     * is not possible on macOS as the native API does not allow it.
     *
...
#    endif
    ...
    /**
     * @brief Get the priority of the current process using the current
     ...
     * the current process, as a member of the enum `BS::os_process_priority`.
     * If the returned object does not contain a value, then either the priority
...
#    elif defined(__linux__) || defined(__APPLE__)
        ...
        // Windows, so instead we get the "nice" value. The usual range is -20
        // to 19 or 20, with higher values corresponding to lower priorities.
        // However, we are only using 6 pre-defined values for portability, so
        // if the value was set via any means other than
        // `BS::set_os_process_priority()`, it may not match one of our
        // pre-defined values. Note that `getpriority()` returns -1 on error,
        // but since this does not correspond to any of our pre-defined values,
        // this function will return `std::nullopt` anyway.
        const int nice_val =
            getpriority(PRIO_PROCESS, static_cast<id_t>(getpid()));
        switch (nice_val)
        {
...
#    endif
    ...
    /**
     * @brief Set the priority of the current process using the current
     * platform's native API. This should work on Windows, Linux, and macOS.
...
#    if defined(_WIN32)
        ...
        return SetPriorityClass(GetCurrentProcess(),
                                static_cast<DWORD>(priority))
            != 0;
#    elif defined(__linux__) || defined(__APPLE__)
        // On Linux/macOS there is no direct analogue of `SetPriorityClass()` on
        // Windows, so instead we set the "nice" value. The usual range is -20
        // to 19 or 20, with higher values corresponding to lower priorities.
        // However, we are only using 6 pre-defined values for portability. Note
        // that the "nice" values are only relevant for the `SCHED_OTHER`
        // policy, but we do not set that policy here, as it is per-thread
        // rather than per-process. Also, it's important to note that a non-root
        // user cannot decrease the nice value (i.e. increase the process
        // priority), only increase it. This can cause confusing behavior. For
        // example, if the current priority is `BS::os_process_priority::normal`
        // and the user sets it to `BS::os_process_priority::idle`, they cannot
        // change it back `BS::os_process_priority::normal`.
...
#    endif
    ...
    /**
     * @brief A class used to obtain information about the current thread and,
     * if native extensions are enabled, set its priority and affinity.
     */
    ...
    public:
        /**
         * @brief Get the index of the current thread. If this thread belongs to
         * a `BS::thread_pool` object, the return value will be an index in the
         * range `[0, N)` where `N == BS::thread_pool::get_thread_count()`.
         * Otherwise, for example if this thread is the main thread or an
         * independent thread not in any pools, `std::nullopt` will be returned.
         *
         * @return An `std::optional` object, optionally containing a thread
         * index.
         */
        ...
        /**
         * @brief Get a pointer to the thread pool that owns the current thread.
         * If this thread belongs to a `BS::thread_pool` object, the return
         * value will be a `void` pointer to that object. Otherwise, for example
         * if this thread is the main thread or an independent thread not in any
         * pools, `std::nullopt` will be returned.
         *
         * @return An `std::optional` object, optionally containing a pointer to
         * a thread pool. Note that this will be a `void` pointer, so it must be
         * cast to the desired instantiation of the `BS::thread_pool` template
         * in order to use any member functions.
         */
...
#ifdef BS_THREAD_POOL_NATIVE_EXTENSIONS
        /**
         * @brief Get the processor affinity of the current thread using the
         * current platform's native API. This should work on Windows and Linux,
         * but is not possible on macOS as the native API does not allow it.
         *
         * @return An `std::optional` object, optionally containing the
         * processor affinity of the current thread as an `std::vector<bool>`
         * where each element corresponds to a logical processor. If the
         * returned object does not contain a value, then the affinity could not
         * be determined. On macOS, this function always returns `std::nullopt`.
         */
...
#    if defined(_WIN32)
            ...
            // `SetThreadAffinityMask()` returns the previous affinity mask, so
            // we can use that to get the current affinity and then restore it.
            // It's a bit of a hack, but it works. Since the thread affinity
            // must be a subset of the process affinity, we use the process
            // affinity as the temporary value.
            DWORD_PTR process_mask = 0;
            DWORD_PTR system_mask = 0;
            if (GetProcessAffinityMask(GetCurrentProcess(), &process_mask,
                                       &system_mask)
...
#    endif
        ...
        /**
         * @brief Set the processor affinity of the current thread using the
         * current platform's native API. This should work on Windows and Linux,
         * but is not possible on macOS as the native API does not allow it.
         * Note that the thread affinity must be a subset of the process
         * affinity (as obtained using `BS::get_os_process_affinity()`) for the
         * containing process of a thread.
         *
         * @param affinity The processor affinity to set, as an
         * `std::vector<bool>` where each element corresponds to a logical
         * processor.
         * @return `true` if the affinity was set successfully, `false`
         * otherwise. On macOS, this function always returns `false`.
         */
...
#    if defined(_WIN32)
            ...
            for (std::size_t i = 0; i < std::min<std::size_t>(
                                        affinity.size(), sizeof(DWORD_PTR) * 8);
                 ++i)
...
#    endif
        ...
        /**
         * @brief Get the name of the current thread using the current
         * platform's native API. This should work on Windows, Linux, and macOS.
         *
         * @return An `std::optional` object, optionally containing the name of
         * the current thread. If the returned object does not contain a value,
         * then the name could not be determined.
         */
        [[nodiscard]] static std::optional<std::string> get_os_thread_name()
        {
#    if defined(_WIN32)
            // On Windows thread names are wide strings, so we need to convert
            ...
            if (data == nullptr)
                return std::nullopt;
            const int size = WideCharToMultiByte(CP_UTF8, 0, data, -1, nullptr,
                                                 0, nullptr, nullptr);
            if (size == 0)
            {
                LocalFree(data);
            ...
            const int result = WideCharToMultiByte(
                CP_UTF8, 0, data, -1, name.data(), size, nullptr, nullptr);
            LocalFree(data);
            if (result == 0)
                return std::nullopt;
            return name;
#    elif defined(__linux__) || defined(__APPLE__)
#        ifdef __linux__
            // On Linux thread names are limited to 16 characters, including the
            // null terminator.
            constexpr std::size_t buffer_size = 16;
#        else
            // On macOS thread names are limited to 64 characters, including the
            // null terminator.
...
#    endif
        ...
        /**
         * @brief Set the name of the current thread using the current
         * platform's native API. This should work on Windows, Linux, and macOS.
         * Note that on Linux thread names are limited to 16 characters,
...
#    if defined(_WIN32)
            ...
            if (MultiByteToWideChar(CP_UTF8, 0, name.data(), -1, wide.data(),
                                    size)
                == 0)
                return false;
            const HRESULT hr =
...
#    elif defined(__APPLE__)
            // On macOS, unlike Linux, a thread can only set a name for itself,
...
#    endif
         ...
         * platform's native API. This should work on Windows, Linux, and macOS.
         *
         * @return An `std::optional` object, optionally containing the priority
         * of the current thread, as a member of the enum
         * `BS::os_thread_priority`. If the returned object does not contain a
         * value, then either the priority could not be determined, or it is not
...
#    elif defined(__linux__)
            // On Linux, we distill the choices of scheduling policy, priority,
            // and "nice" value into 7 pre-defined levels, for simplicity and
            // portability. The total number of possible combinations of
            // policies and priorities is much larger, so if the value was set
            // via any means other than
            // `BS::this_thread::set_os_thread_priority()`, it may not match one
            // of our pre-defined values.
            int policy = 0;
            ...
            {
                // The only pre-defined priority that uses SCHED_FIFO and the
                // maximum available priority value is the "realtime" priority.
                    ...
                    == sched_get_priority_min(SCHED_RR)
                        + (sched_get_priority_max(SCHED_RR)
                           - sched_get_priority_min(SCHED_RR))
                            / 2)
            {
                // The only pre-defined priority that uses SCHED_RR and a
                // priority in the middle of the available range is the
                // "highest" priority.
...
#        ifdef __linux__
            ...
            {
                // The only pre-defined priority that uses SCHED_IDLE is the
                // "idle" priority. Note that this scheduling policy is not
                // available on macOS.
...
#        endif
            ...
            {
                // For SCHED_OTHER, the result depends on the "nice" value. The
                // usual range is -20 to 19 or 20, with higher values
                // corresponding to lower priorities. Note that `getpriority()`
                // returns -1 on error, but since this does not correspond to
                // any of our pre-defined values, this function will return
                // `std::nullopt` anyway.
                const int nice_val = getpriority(
                    PRIO_PROCESS, static_cast<id_t>(syscall(SYS_gettid)));
                switch (nice_val)
                {
                case PRIO_MIN + 2:
...
#        endif
            ...
            return std::nullopt;
#    elif defined(__APPLE__)
            // On macOS, we distill the choices of scheduling policy and
            // priority into 7 pre-defined levels, for simplicity and
            // portability. The total number of possible combinations of
            // policies and priorities is much larger, so if the value was set
            // via any means other than
            // `BS::this_thread::set_os_thread_priority()`, it may not match one
            ...
            if (policy == SCHED_OTHER)
            {
                // For SCHED_OTHER, the result depends on the specific value of
                // the priority.
...
#    endif
         ...
         * platform's native API. This should work on Windows, Linux, and macOS.
         * However, note that higher priorities might require elevated
         * permissions.
         *
         * @param priority The priority to set. Must be a value from the enum
         * `BS::os_thread_priority`.
         * @return `true` if the priority was set successfully, `false`
         * otherwise. Usually, `false` means that the user does not have the
         * necessary permissions to set the desired priority.
         */
...
#    elif defined(__linux__)
            ...
            // and "nice" value into 7 pre-defined levels, for simplicity and
            // portability. The total number of possible combinations of
            // policies and priorities is much larger, but allowing more
            // fine-grained control would not be portable.
            int policy = 0;
            struct sched_param param = {};
            ...
            case os_thread_priority::realtime:
                // "Realtime" pre-defined priority: We use the policy
                // `SCHED_FIFO` with the highest possible priority.
                policy = SCHED_FIFO;
                param.sched_priority = sched_get_priority_max(SCHED_FIFO);
                break;
            case os_thread_priority::highest:
                // "Highest" pre-defined priority: We use the policy `SCHED_RR`
                // ("round-robin") with a priority in the middle of the
                // available range.
                policy = SCHED_RR;
            ...
            case os_thread_priority::above_normal:
                ...
                // `SCHED_OTHER` (the default). This policy does not accept a
                // priority value, so priority must be 0. However, we set the
                // "nice" value to the minimum value as given by `PRIO_MIN`,
                // plus 2 (which should evaluate to -18). The usual range is -20
                // to 19 or 20, with higher values corresponding to lower
                // priorities.
                policy = SCHED_OTHER;
            ...
            case os_thread_priority::normal:
                // "Normal" pre-defined priority: We use the policy
                // `SCHED_OTHER`, priority must be 0, and we set the "nice"
            ...
            case os_thread_priority::below_normal:
                ...
                // `SCHED_OTHER`, priority must be 0, and we set the "nice"
                // value to half the maximum value as given by `PRIO_MAX`,
            ...
            case os_thread_priority::lowest:
                // "Lowest" pre-defined priority: We use the policy
                // `SCHED_OTHER`, priority must be 0, and we set the "nice"
                // value to the maximum value as given by `PRIO_MAX`, minus 3
            ...
            case os_thread_priority::idle:
                ...
                // `SCHED_IDLE`, priority must be 0, and we don't touch the
                // "nice" value.
                policy = SCHED_IDLE;
                param.sched_priority = 0;
                break;
            ...
            return success;
#    elif defined(__APPLE__)
            // On macOS, unlike Linux, the "nice" value is per-process, not
            // per-thread (in compliance with the POSIX standard). However,
            // unlike Linux, `SCHED_OTHER` on macOS does have a range of
            // priorities. So for `realtime` and `highest` priorities we use
            // `SCHED_FIFO` and `SCHED_RR` respectively as for Linux, but for
            // the other priorities we use `SCHED_OTHER` with a priority in the
            // range given by `sched_get_priority_min(SCHED_OTHER)` to
            // `sched_get_priority_max(SCHED_OTHER)`.
            int policy = 0;
            struct sched_param param = {};
            switch (priority)
            ...
            case os_thread_priority::above_normal:
                // "Above normal" pre-defined priority: We use the policy
                // `SCHED_OTHER` (the default) with the highest possible
                // priority.
                policy = SCHED_OTHER;
            ...
            case os_thread_priority::normal:
                // "Normal" pre-defined priority: We use the policy
                // `SCHED_OTHER` (the default) with a priority in the middle of
                // the available range (which appears to be the default?).
                policy = SCHED_OTHER;
            ...
            case os_thread_priority::below_normal:
                // "Below normal" pre-defined priority: We use the policy
                // `SCHED_OTHER` (the default) with a priority equal to 2/3rds
                // of the normal value.
                policy = SCHED_OTHER;
                param.sched_priority = sched_get_priority_min(SCHED_OTHER)
                    + (sched_get_priority_max(SCHED_OTHER)
            ...
            case os_thread_priority::lowest:
                // "Lowest" pre-defined priority: We use the policy
                // `SCHED_OTHER` (the default) with a priority equal to 1/3rd of
                // the normal value.
                policy = SCHED_OTHER;
                param.sched_priority = sched_get_priority_min(SCHED_OTHER)
                    + (sched_get_priority_max(SCHED_OTHER)
            ...
            case os_thread_priority::idle:
                ...
                // `SCHED_OTHER` (the default) with the lowest possible
                // priority.
                policy = SCHED_OTHER;
                param.sched_priority = sched_get_priority_min(SCHED_OTHER);
                break;
...
#    endif
    ...
    // The common type of two signed integers is the larger of the integers,
                         ...
                         || (std::is_unsigned_v<T1> && std::is_signed_v<T2>)>>
    {
        ...
        using type = std::conditional_t<
            ...
            // signed type that can hold both of them. If both are 8 bits, or
            // the signed type is 16 bits and the unsigned type is 8 bits, the
            // common type is `std::int16_t`. Otherwise, if both are 16 bits, or
            // the signed type is 32 bits and the unsigned type is smaller, the
            // common type is `std::int32_t`. Otherwise, if both are 32 bits or
            ...
            // If the unsigned integer is 64 bits, the common type should also
            // be an unsigned 64-bit integer, that is, `std::uint64_t`. The
            // reason is that the most common scenario where this might happen
            // is where the indices go from 0 to `x` where `x` has been
            // previously defined as `std::size_t`, e.g. the size of a vector.
            // Note that this will fail if the first index is negative; in that
            // case, the user must cast the indices explicitly to the desired
            // common type. If the unsigned integer is not 64 bits, then the
            // signed integer must be 64 bits, hence the common type is
            // `std::int64_t`.
    ...
    /**
     * @brief An enumeration of flags to be used in the bitmask template
    ...
    public:
         ...
         * implementation. This is usually determined by the number of cores in
         * the CPU. If a core is hyperthreaded, it will count as two threads.
         */
        thread_pool()
            : thread_pool(0, [] {})
        {}
        ...
        /**
         * @brief Construct a new thread pool with the specified number of
         * threads.
         *
         ...
         * function.
         *
         * @param init An initialization function to run in each thread before
         * it starts executing any submitted tasks. The function must have no
         * return value, and can either take one argument, the thread index of
         * type `std::size_t`, or zero arguments. It will be executed exactly
         * once per thread, when the thread is first constructed. The
         * initialization function must not throw any exceptions, as that will
         * result in program termination. Any exceptions must be handled
         * explicitly within the function.
         */
        template <BS_THREAD_POOL_INIT_FUNC_CONCEPT(F)>
        explicit thread_pool(F &&init)
            : thread_pool(0, std::forward<F>(init))
        {}

        /**
         * @brief Construct a new thread pool with the specified number of
         * threads and initialization function.
         *
         * @param num_threads The number of threads to use.
         * @param init An initialization function to run in each thread before
         * it starts executing any submitted tasks. The function must have no
        ...
        // The thread pool cannot be copied or moved.
        ...
        /**
         * @brief Destruct the thread pool. Waits for all tasks to complete,
         * then destroys all threads. If a cleanup function was set, it will run
         * in each thread right before it is destroyed. Note that if the pool is
         * paused, then any tasks still in the queue will never be executed.
         */
        ...
        /**
         * @brief Parallelize a loop by automatically splitting it into blocks
         * and submitting each block separately to the queue, with the specified
         * priority. The block function takes two arguments, the start and end
         * of the block, so that it is only called once per block, but it is up
         * to the user make sure the block function correctly deals with all the
         * indices in each block. Does not return a `BS::multi_future`, so the
         * user must use `wait()` or some other method to ensure that the loop
         * finishes executing, otherwise bad things will happen.
         *
         * @tparam T1 The type of the first index. Should be a signed or
         * unsigned integer.
         * @tparam T2 The type of the index after the last index. Should be a
         * signed or unsigned integer.
         * @tparam F The type of the function to loop through.
         * @param first_index The first index in the loop.
         * @param index_after_last The index after the last index in the loop.
         * The loop will iterate from `first_index` to `(index_after_last - 1)`
         * inclusive. In other words, it will be equivalent to `for (T i =
         * first_index; i < index_after_last; ++i)`. Note that if
         * `index_after_last <= first_index`, no blocks will be submitted.
         * @param block A function that will be called once per block. Should
         * take exactly two arguments: the first index in the block and the
         * index after the last index in the block. `block(start, end)` should
         * typically involve a loop of the form `for (T i = start; i < end;
         * ++i)`.
         * @param num_blocks The maximum number of blocks to split the loop
         * into. The default is 0, which means the number of blocks will be
         * equal to the number of threads in the pool.
         * @param priority The priority of the tasks. Should be between -128 and
         * +127 (a signed 8-bit integer). The default is 0. Only taken into
         * account if the flag `BS:tp::priority` is enabled in the template
                           ...
                           const priority_t priority = 0)
        {
            if (static_cast<T>(index_after_last) > static_cast<T>(first_index))
            {
                ...
                {
                    detach_task(
                        [block_ptr, start = blks.start(blk),
         ...
         * @brief Parallelize a loop by automatically splitting it into blocks
         * and submitting each block separately to the queue, with the specified
         * priority. The loop function takes one argument, the loop index, so
         * that it is called many times per block. Does not return a
         * `BS::multi_future`, so the user must use `wait()` or some other
         * method to ensure that the loop finishes executing, otherwise bad
         * things will happen.
         *
         * @tparam T1 The type of the first index. Should be a signed or
         * unsigned integer.
         ...
         * first_index; i < index_after_last; ++i)`. Note that if
         * `index_after_last <= first_index`, no blocks will be submitted.
         * @param loop The function to loop through. Will be called once per
         * index, many times per block. Should take exactly one argument: the
         * loop index.
         * @param num_blocks The maximum number of blocks to split the loop
         * into. The default is 0, which means the number of blocks will be
         * equal to the number of threads in the pool.
        ...
        template <typename T1, typename T2,
                  typename T = common_index_type_t<T1, T2>, typename F>
        void detach_loop(const T1 first_index, const T2 index_after_last,
         ...
         * with the specified priority. The sequence function takes one
         * argument, the task index, and will be called once per index. Does not
         * return a `BS::multi_future`, so the user must use `wait()` or some
         * other method to ensure that the sequence finishes executing,
         ...
         * @param index_after_last The index after the last index in the
         * sequence. The sequence will iterate from `first_index` to
         * `(index_after_last - 1)` inclusive. In other words, it will be
         * equivalent to `for (T i = first_index; i < index_after_last; ++i)`.
         * Note that if `index_after_last <= first_index`, no tasks will be
         * submitted.
         * @param sequence The function used to define the sequence. Will be
         * called once per index. Should take exactly one argument, the index.
         * @param priority The priority of the tasks. Should be between -128 and
         * +127 (a signed 8-bit integer). The default is 0. Only taken into
         * account if the flag `BS:tp::priority` is enabled in the template
         * parameter, otherwise has no effect.
        ...
        /**
         * @brief Submit a function with no arguments and no return value into
         * the task queue, with the specified priority. To submit a function
         * with arguments, enclose it in a lambda expression. Does not return a
         * future, so the user must use `wait()` or some other method to ensure
         * that the task finishes executing, otherwise bad things will happen.
         *
         * @tparam F The type of the function.
         * @param task The function to submit.
         * @param priority The priority of the task. Should be between -128 and
         * +127 (a signed 8-bit integer). The default is 0. Only taken into
         * account if the flag `BS:tp::priority` is enabled in the template
         * parameter, otherwise has no effect.
         */
        ...
        {
            std::vector<thread_t::native_handle_type> native_handles(
                thread_count);
        ...
        /**
         * @brief Get the number of tasks currently being executed by the
         * threads.
         *
        ...
        /**
         * @brief Get the total number of unfinished tasks: either still waiting
         * in the queue, or running in a thread. Note that `get_tasks_total() ==
        ...
        /**
         * @brief Pause the pool. The workers will temporarily stop retrieving
         * new tasks out of the queue, although any tasks already executed will
         * keep running until they are finished. Only enabled if the flag
        ...
        /**
         * @brief Purge all the tasks waiting in the queue. Tasks that are
         * currently running will not be affected, but any tasks still waiting
         * in the queue will be discarded, and will never be executed by the
         * threads. Please note that there is no way to restore the purged
         * tasks.
         */
        void purge()
        {
         ...
         * running tasks to be completed, then destroys all threads in the pool
         * and creates a new thread pool with the new number of threads. Any
         * tasks that were waiting in the queue before the pool was reset will
         * then be executed by the new threads. If the pool was paused before
         ...
         * currently running tasks to be completed, then destroys all threads in
         * the pool and creates a new thread pool with the new number of
         * threads. Any tasks that were waiting in the queue before the pool was
         * reset will then be executed by the new threads. If the pool was
        ...
        /**
         * @brief Reset the pool with the total number of hardware threads
         * available, as reported by the implementation, and a new
         * initialization function. Waits for all currently running tasks to be
         * completed, then destroys all threads in the pool and creates a new
         * thread pool with the new number of threads and initialization
         * function. Any tasks that were waiting in the queue before the pool
         * was reset will then be executed by the new threads. If the pool was
         * paused before resetting it, the new pool will be paused as well.
         *
         * @param init An initialization function to run in each thread before
         * it starts executing any submitted tasks. The function must have no
         ...
         *
         * @param cleanup A cleanup function to run in each thread right before
         * it is destroyed, which will happen when the pool is destructed or
         * reset. The function must have no return value, and can either take
         * one argument, the thread index of type `std::size_t`, or zero
         * arguments. The cleanup function must not throw any exceptions, as
         * that will result in program termination. Any exceptions must be
         * handled explicitly within the function.
         */
        template <BS_THREAD_POOL_INIT_FUNC_CONCEPT(F)>
        void set_cleanup_func(F &&cleanup)
        {
            ...
            {
                cleanup_func = [cleanup = std::forward<F>(cleanup)](
                                   std::size_t) { cleanup(); };
            }
        }
         ...
         * of the block, so that it is only called once per block, but it is up
         * to the user make sure the block function correctly deals with all the
         * indices in each block. Returns a `BS::multi_future` that contains the
         * futures for all of the blocks.
         *
         * @tparam T1 The type of the first index. Should be a signed or
         * unsigned integer.
         * @tparam T2 The type of the index after the last index. Should be a
         ...
         * inclusive. In other words, it will be equivalent to `for (T i =
         * first_index; i < index_after_last; ++i)`. Note that if
         * `index_after_last <= first_index`, no blocks will be submitted, and
         * an empty `BS::multi_future` will be returned.
         * @param block A function that will be called once per block. Should
         * take exactly two arguments: the first index in the block and the
         * index after the last index in the block. `block(start, end)` should
         * typically involve a loop of the form `for (T i = start; i < end;
         ...
         * account if the flag `BS:tp::priority` is enabled in the template
         * parameter, otherwise has no effect.
         * @return A `BS::multi_future` that can be used to wait for all the
         * blocks to finish. If the block function returns a value, the
         * `BS::multi_future` can also be used to obtain the values returned by
         * each block.
         */
        template <typename T1, typename T2,
                  typename T = common_index_type_t<T1, T2>, typename F,
        ...
        submit_blocks(const T1 first_index, const T2 index_after_last,
                      F &&block, const std::size_t num_blocks = 0,
                      const priority_t priority = 0)
        {
            if (static_cast<T>(index_after_last) > static_cast<T>(first_index))
         ...
         * and submitting each block separately to the queue, with the specified
         * priority. The loop function takes one argument, the loop index, so
         * that it is called many times per block. It must have no return value.
         ...
         * inclusive. In other words, it will be equivalent to `for (T i =
         * first_index; i < index_after_last; ++i)`. Note that if
         * `index_after_last <= first_index`, no tasks will be submitted, and an
         * empty `BS::multi_future` will be returned.
         * @param loop The function to loop through. Will be called once per
         * index, many times per block. Should take exactly one argument: the
         * loop index. It cannot have a return value.
         * @param num_blocks The maximum number of blocks to split the loop
         * into. The default is 0, which means the number of blocks will be
         * equal to the number of threads in the pool.
         * @param priority The priority of the tasks. Should be between -128 and
         ...
         * @brief Submit a sequence of tasks enumerated by indices to the queue,
         * with the specified priority. The sequence function takes one
         * argument, the task index, and will be called once per index. Returns
         * a `BS::multi_future` that contains the futures for all of the tasks.
         *
         * @tparam T1 The type of the first index. Should be a signed or
         * unsigned integer.
         * @tparam T2 The type of the index after the last index. Should be a
         ...
         * equivalent to `for (T i = first_index; i < index_after_last; ++i)`.
         * Note that if `index_after_last <= first_index`, no tasks will be
         * submitted, and an empty `BS::multi_future` will be returned.
         * @param sequence The function used to define the sequence. Will be
         * called once per index. Should take exactly one argument, the index.
         * @param priority The priority of the tasks. Should be between -128 and
         * +127 (a signed 8-bit integer). The default is 0. Only taken into
         ...
         * parameter, otherwise has no effect.
         * @return A `BS::multi_future` that can be used to wait for all the
         * tasks to finish. If the sequence function returns a value, the
         * `BS::multi_future` can also be used to obtain the values returned by
         * each task.
         */
        template <typename T1, typename T2,
        ...
        [[nodiscard]] multi_future<R>
        submit_sequence(const T1 first_index, const T2 index_after_last,
                        F &&sequence, const priority_t priority = 0)
        {
            if (static_cast<T>(index_after_last) > static_cast<T>(first_index))
            {
                ...
                multi_future<R> future;
                future.reserve(
                    static_cast<std::size_t>(static_cast<T>(index_after_last)
                                             > static_cast<T>(first_index)));
                for (T i = static_cast<T>(first_index);
                     i < static_cast<T>(index_after_last); ++i)
                {
                    future.push_back(submit_task(
                        [sequence_ptr, i] { return (*sequence_ptr)(i); },
                        priority));
                }
                return future;
            }
        ...
        /**
         * @brief Submit a function with no arguments into the task queue, with
         * the specified priority. To submit a function with arguments, enclose
         * it in a lambda expression. If the function has a return value, get a
         * future for the eventual returned value. If the function has no return
         * value, get an `std::future<void>` which can be used to wait until the
         * task finishes.
         *
         * @tparam F The type of the function.
...
#    define BS_THREAD_POOL_PROMISE_MEMBER_ACCESS promise->
#endif
            std::future<R> future =
                BS_THREAD_POOL_PROMISE_MEMBER_ACCESS get_future();
            detach_task(
        ...
        /**
         * @brief Unpause the pool. The workers will resume retrieving new tasks
         * out of the queue. Only enabled if the flag `BS:tp::pause` is enabled
        ...
        /**
         * @brief Wait for tasks to be completed. Normally, this function waits
         * for all tasks, both those that are currently running in the threads
         * and those that are still waiting in the queue. However, if the pool
         * is paused, this function only waits for the currently running tasks
         * (otherwise it would wait forever). Note: To wait for just one
         * specific task, use `submit_task()` instead, and call the `wait()`
         * member function of the generated future.
         *
         * @throws `wait_deadlock` if called from within a thread of the same
         * pool, which would result in a deadlock. Only enabled if the flag
         ...
         * point.
         * @param timeout_time The time point at which to stop waiting.
         * @return `true` if all tasks finished running, `false` if the time
         * point was reached but some tasks are still running.
         * @throws `wait_deadlock` if called from within a thread of the same
         * pool, which would result in a deadlock. Only enabled if the flag
         * `BS:tp::wait_deadlock_checks` is enabled in the template parameter.
         */
    ...
    private:
        ...
        /**
         * @brief Create the threads in the pool and assign a worker to each
         * thread.
         *
         * @param num_threads The number of threads to use.
         * @param init An initialization function to run in each thread before
         * it starts executing any submitted tasks.
         */
        template <typename F>
         ...
         *
         * @param num_threads The parameter passed to the constructor or
         * `reset()`. If the parameter is a positive number, then the pool will
         * be created with this number of threads. If the parameter is
         * non-positive, or a parameter was not supplied (in which case it will
         * have the default value of 0), then the pool will be created with the
         * total number of hardware threads available, as obtained from
         * `thread_t::hardware_concurrency()`. If the latter returns zero for
         * some reason, then the pool will be created with just one thread.
         ...
         * reset, while the public member function `reset()` also handles the
         * case where the pool is paused.
         *
         * @param num_threads The number of threads to use.
         * @param init An initialization function to run in each thread before
         * it starts executing any submitted tasks.
         ...
         * Waits until it is notified by `detach_task()` that a task is
         * available, and then retrieves the task from the queue and executes
         * it. Once the task finishes, the worker notifies `wait()` in case it
         * is waiting.
         *
        ...
        void worker(BS_THREAD_POOL_WORKER_TOKEN const std::size_t idx)
        {
            ...
            while (true)
            {
                ...
                task_available_cv.wait(
                    tasks_lock BS_THREAD_POOL_WAIT_TOKEN, [this] {
                        if constexpr (pause_enabled)
                            return !(paused || tasks.empty())
                                BS_THREAD_POOL_OR_STOP_CONDITION;
                        else
                            return !tasks.empty()
                                        BS_THREAD_POOL_OR_STOP_CONDITION;
                    });
                ...
                {
                    task_t task =
                        pop_task(); // NOLINT(misc-const-correctness) In C++23
         ...
         * destroyed, which will happen when the pool is destructed or reset.
         * The function must have no return value, and can either take one
         * argument, the thread index of type `std::size_t`, or zero arguments.
         * The cleanup function must not throw any exceptions, as that will
         * result in program termination. Any exceptions must be handled
         * explicitly within the function. The default is an empty function,
         * i.e., no cleanup will be performed.
         */
        function_t<void(std::size_t)> cleanup_func = [](std::size_t) {};

        /**
         * @brief An initialization function to run in each thread before it
         * starts executing any submitted tasks. The function must have no
         * return value, and can either take one argument, the thread index of
         * type `std::size_t`, or zero arguments. It will be executed exactly
         * once per thread, when the thread is first constructed. The
         * initialization function must not throw any exceptions, as that will
         * result in program termination. Any exceptions must be handled
         * explicitly within the function. The default is an empty function,
         * i.e., no initialization will be performed.
         */
        function_t<void(std::size_t)> init_func = [](std::size_t) {};

        /**
         * @brief A flag indicating whether the workers should pause. When set
         * to `true`, the workers temporarily stop retrieving new tasks out of
         * the queue, although any tasks already executed will keep running
         * until they are finished. When set to `false` again, the workers
         * resume retrieving tasks. Only enabled if the flag `BS:tp::pause` is
 ...
 */
        ...
        /**
         * @brief A condition variable to notify `wait()` that the tasks are
         * done.
         */
        ...
        /**
         * @brief A smart pointer to manage the memory allocated for the
         * threads.
         */
    ...
    public:
        ...
        /**
         * @brief Print any number of items into the output stream. Ensures that
         * no other threads print to this stream simultaneously, as long as they
         * all exclusively use the same `BS::synced_stream` object to print.
         *
        ...
        /**
         * @brief Print any number of items into the output stream, followed by
         * a newline character. Ensures that no other threads print to this
         * stream simultaneously, as long as they all exclusively use the same
        ...
        /**
         * @brief A stream manipulator to pass to a `BS::synced_stream` (an
         * explicit cast of `std::endl`). Prints a newline character to the
         * stream, and then flushes it. Should only be used if flushing is
         * desired, otherwise a newline character should be used instead.
         */
        ...
        /**
         * @brief A stream manipulator to pass to a `BS::synced_stream` (an
         * explicit cast of `std::flush`). Used to flush the stream.
         */
     ...
     * primitive that allows more than one concurrent access to the same
     * resource. The number of concurrent accessors is limited by the
     * semaphore's counter, which is decremented when a thread acquires the
     * semaphore and incremented when a thread releases the semaphore. If the
     * counter is zero, a thread trying to acquire the semaphore will be blocked
    ...
    class [[nodiscard]] counting_semaphore
    {
        static_assert(LeastMaxValue >= 0,
                      "The least maximum value for a counting semaphore must "
                      "not be negative.");

    public:
        /**
         * @brief Construct a new counting semaphore with the given initial
         * counter value.
         *
        ...
        // The copy and move constructors and assignment operators are deleted.
        // The semaphore cannot be copied or moved.
        ...
        /**
         * @brief Returns the internal counter's maximum possible value, which
         * in this implementation is equal to `LeastMaxValue`.
         *
        ...
        /**
         * @brief Atomically decrements the internal counter by 1 if it is
        ...
        /**
         * @brief Atomically increments the internal counter. Any thread(s)
         * waiting for the counter to be greater than 0, such as due to being
        ...
        /**
         * @brief Tries to atomically decrement the internal counter by 1 if it
         * is greater than 0; no blocking occurs regardless.
         *
         * @return `true` if decremented the internal counter, `false`
         * otherwise.
         */
        bool try_acquire()
        {
        ...
        /**
         * @brief Tries to atomically decrement the internal counter by 1 if it
         * is greater than 0; otherwise blocks until it is greater than 0 and
         * can successfully decrement the internal counter, or the `rel_time`
         * duration has been exceeded.
         *
         * @tparam Rep An arithmetic type representing the number of ticks to
         * wait.
         * @tparam Period An `std::ratio` representing the length of each tick
         * in seconds.
         ...
         * time point.
         * @param abs_time The earliest time the function must wait until. Note
         * that the function may wait for longer.
         * @return `true` if decremented the internal counter, `false`
         * otherwise.
         */
    ...
    /**
     * @brief A polyfill for `std::binary_semaphore`, to be used if C++20
