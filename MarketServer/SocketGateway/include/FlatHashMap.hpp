...
namespace ska
{
    ...
    namespace detailv3
    {
        ...
        struct functor_storage : Functor
        {
            ...
            {
                return static_cast<const Functor &>(*this)(
                    std::forward<Args>(args)...);
            }
        };
        ...
        struct KeyOrValueEquality : functor_storage<bool, key_equal>
        {
            ...
            {
                return static_cast<equality_storage &>(*this)(lhs.first,
                                                              rhs.first);
            }
            template <typename F, typename S>
            ...
            bool operator()(const std::pair<FL, SL> &lhs,
                            const std::pair<FR, SR> &rhs)
            {
                return static_cast<equality_storage &>(*this)(lhs.first,
                                                              rhs.first);
            }
        ...
        template <typename T, typename FindKey, typename ArgumentHash,
                  typename Hasher, typename ArgumentEqual, typename Equal,
        ...
        public:
            ...
            sherwood_v3_table(size_type bucket_count,
                ...
                : sherwood_v3_table(bucket_count, ArgumentHash(),
                                    ArgumentEqual(), alloc)
            {}
            ...
            sherwood_v3_table(It first, It last, size_type bucket_count,
                              ...
                              const ArgumentAlloc &alloc)
                : sherwood_v3_table(first, last, bucket_count, hash,
                                    ArgumentEqual(), alloc)
            {}
            ...
            sherwood_v3_table(std::initializer_list<T> il,
                              size_type bucket_count,
                              const ArgumentAlloc &alloc)
                : sherwood_v3_table(il, bucket_count, ArgumentHash(),
                                    ArgumentEqual(), alloc)
            {}
            sherwood_v3_table(std::initializer_list<T> il,
                              size_type bucket_count, const ArgumentHash &hash,
                              const ArgumentAlloc &alloc)
                : sherwood_v3_table(il, bucket_count, hash, ArgumentEqual(),
                                    alloc)
            {}
            ...
            sherwood_v3_table &operator=(const sherwood_v3_table &other)
            {
                ...
                clear();
                if (AllocatorTraits::propagate_on_container_copy_assignment::
                        value)
                {
            ...
            {
                return { entries
                         + static_cast<ptrdiff_t>(num_slots_minus_one
                                                  + max_lookups) };
            }
            ...
            void rehash(size_t num_buckets)
            {
                ...
                for (EntryPointer it = new_buckets; it != special_end_item;
                     ++it)
                    it->distance_from_desired = -1;
                special_end_item->distance_from_desired =
                ...
                for (EntryPointer it = new_buckets,
                         ...
                         + static_cast<ptrdiff_t>(num_buckets
                                                  + old_max_lookups);
                     it != end; ++it)
                {
                    if (it->has_value())
            ...
            iterator erase(const_iterator begin_it, const_iterator end_it)
            {
                ...
                ptrdiff_t num_to_move =
                    std::min(static_cast<ptrdiff_t>(
                                 end_it.current->distance_from_desired),
                             end_it.current - begin_it.current);
                EntryPointer to_return = end_it.current - num_to_move;
                for (EntryPointer it = end_it.current;
                     !it->is_at_desired_position();)
                {
                    ...
                    ++it;
                    num_to_move = std::min(
                        static_cast<ptrdiff_t>(it->distance_from_desired),
                        num_to_move);
                }
                return { to_return };
            }
            ...
            {
                return (AllocatorTraits::max_size(*this) - min_lookups)
                    / sizeof(Entry);
            }
        ...
        private:
            ...
            emplace_new_key(int8_t distance_from_desired,
                            EntryPointer current_entry, Key &&key,
                            Args &&...args)
            {
                using std::swap;
                if (num_slots_minus_one == 0
                    || distance_from_desired == max_lookups
    ...
    struct prime_number_hash_policy
    {
        ...
        mod_function next_size_over(size_t &size) const
        {
            ...
            // you now have large gaps which you would hit if somebody called
            // reserve() with an unlucky number.
            // 4. to fill the gaps for every prime p go to wolfram alpha and get
            // ClosestPrime(p * 2^(1/3)) and ClosestPrime(p * 2^(2/3)) and put
            // those in the gaps
            ...
            static constexpr size_t (*const mod_functions[])(
                size_t) = { &mod0,
                            &mod2,
                            &mod3,
                            &mod5,
    ...
    public:
        ...
        {
            return emplace(std::move(key), convertible_to_value())
                .first->second;
        }
        V &at(const K &key)
        {
        ...
        insert_or_assign(typename Table::const_iterator, const key_type &key,
                         M &&m)
        {
        ...
        template <typename M>
        typename Table::iterator
        insert_or_assign(typename Table::const_iterator, key_type &&key, M &&m)
        {
            return insert_or_assign(std::move(key), std::forward<M>(m)).first;
        }

        friend bool operator==(const flat_hash_map &lhs,
                               const flat_hash_map &rhs)
        {
    ...
    public:
        ...
        friend bool operator==(const flat_hash_set &lhs,
                               const flat_hash_set &rhs)
        {
            if (lhs.size() != rhs.size())
                return false;
