...
namespace moodycamel
{
    ...
    enum memory_order
    {
        ...
        // #LoadLoad, #LoadStore, #StoreStore, and most significantly,
        // #StoreLoad
...
#    ifdef AE_VCPP
#        pragma warning(push)
#        pragma warning(disable : 4365) // Disable erroneous 'conversion from
                                        // long to unsigned int, signed/unsigned
...
#    else
    AE_FORCEINLINE void fence(memory_order order) AE_NO_TSAN
    {
        // Non-specialized arch, use heavier memory barriers everywhere just in
        // case :-(
        switch (order)
        {
        case memory_order_relaxed:
...
#ifdef AE_VCPP
#    pragma warning(push)
#    pragma warning(disable : 4100) // Get rid of (erroneous) 'unreferenced
        ...
        {}
#ifdef __cplusplus_cli
        // Work around bug with universal reference/nullptr combination that
        // only appears when /clr is on
...
#    endif
    ...
    private:
        ...
        // optimizations. `volatile` will make memory access slow, but is
    ...
    namespace spsc_sema
    {
        ...
        public:
                ...
                : m_hSema()
            {
                ...
                m_hSema =
                    CreateSemaphoreW(nullptr, initialCount, maxLong, nullptr);
                assert(m_hSema);
            }
        ...
        public:
            ...
            {
                assert(initialCount >= 0);
                m_sema = xSemaphoreCreateCounting(
                    static_cast<UBaseType_t>(~0ull),
            ...
            bool timed_wait(std::uint64_t usecs) AE_NO_TSAN
            {
                ...
                TickType_t ticks =
                    static_cast<TickType_t>(msecs / portTICK_PERIOD_MS);
                if (ticks == 0)
                    return try_wait();
...
#    error Unsupported platform! (No semaphore wrapper available)
        ...
        private:
            ...
            bool
            waitWithPartialSpinning(std::int64_t timeout_usecs = -1) AE_NO_TSAN
            {
                ssize_t oldCount;
                // Is there a better way to set the initial spin count?
                // If we lower it to 1000, testBenaphore becomes 15x slower on
                // my Core i7-5930K Windows PC, as threads start hitting the
                // kernel semaphore.
                int spin = 1024;
                while (--spin >= 0)
                {
                    ...
                    compiler_fence(
                        memory_order_acquire); // Prevent the compiler from
                                               // collapsing the loop.
                }
                ...
                if (timeout_usecs > 0
                    ...
                    return true;
                // At this point, we've timed out waiting for the semaphore, but
                // the count is still decremented indicating we may still be
                // waiting on it. So we have to re-adjust the count, but only if
                // the semaphore wasn't signaled enough times for us too since
                // then. If it was, we need to release the semaphore too.
                while (true)
                {
                    oldCount = m_count.fetch_add_release(1);
                    if (oldCount < 0)
                        return false; // successfully restored things to the way
                                      // they were
                    // Oh, the producer thread just signaled the semaphore after
                    // all. Try again:
                    oldCount = m_count.fetch_add_acquire(-1);
