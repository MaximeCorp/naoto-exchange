...
namespace moodycamel
{
    ...
    public:
        explicit BlockingReaderWriterCircularBuffer(std::size_t capacity)
            ...
            , slots_(new spsc_sema::LightweightSemaphore(
                  static_cast<spsc_sema::LightweightSemaphore::ssize_t>(
                      capacity)))
            ...
            , nextItem(0)
        {
            ...
            mask = capacity++;
            rawData = static_cast<char *>(std::malloc(
                capacity * sizeof(T) + std::alignment_of<T>::value - 1));
            data = align_for<T>(rawData);
        }

        BlockingReaderWriterCircularBuffer(
            BlockingReaderWriterCircularBuffer &&other)
            : maxcap(0)
            , mask(0)
            , rawData(nullptr)
            , data(nullptr)
        ...
        // Blocks the current thread until there's enough space to enqueue the
        // given item, then enqueues it (via copy). Thread-safe when called by
        ...
        // Blocks the current thread until there's enough space to enqueue the
        // given item, then enqueues it (via move, if possible). Thread-safe
        // when called by producer thread. No exception guarantee (state will be
        // corrupted) if constructor of T throws.
        ...
        // Blocks the current thread until there's enough space to enqueue the
        // given item, or the timeout expires. Returns false without enqueueing
        // the item if the timeout expires, otherwise enqueues the item (via
        // copy) and returns true. Thread-safe when called by producer thread.
        // No exception guarantee (state will be corrupted) if constructor of T
        // throws.
        ...
        // given item, or the timeout expires. Returns false without enqueueing
        // the item if the timeout expires, otherwise enqueues the item (via
        // move, if possible) and returns true. Thread-safe when called by
        // producer thread. No exception guarantee (state will be corrupted) if
        // constructor of T throws.
        ...
        // No exception guarantee (state will be corrupted) if assignment
        // operator of U throws.
        template <typename U>
        ...
        // dequeues it. Thread-safe when called by consumer thread. No exception
        // guarantee (state will be corrupted) if assignment operator of U
        // throws.
        template <typename U>
        ...
        // Returns a pointer to the next element in the queue (the one that
        // would be removed next by a call to `try_dequeue` or `try_pop`). If
        // the queue appears empty at the time the method is called, returns
        // nullptr instead. Thread-safe when called by consumer thread.
        inline T *peek()
        {
        ...
        // Returns a (possibly outdated) snapshot of the total number of
        // elements currently in the buffer. Thread-safe.
        inline std::size_t size_approx() const
        {
            return items->availableApprox();
        }

        // Returns the maximum number of elements that this circular buffer can
        // hold at once. Thread-safe.
    ...
    private:
        ...
        void inner_enqueue(U &&item)
        {
            ...
            new (reinterpret_cast<T *>(data) + (i & mask))
    ...
    private:
        ...
        char cachelineFiller0
            [MOODYCAMEL_CACHE_LINE_SIZE - sizeof(char *) * 2
             - sizeof(std::size_t) * 2
